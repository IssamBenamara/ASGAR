...
class IDM(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(IDM, self).__init__(feature_map,
                                     model_id=model_id,
                                     gpu=gpu,
        ...
        self.base_dnn = MLP_Layer(input_dim=input_dim,
                                         ...
                                         batch_norm=batch_norm,
                                         use_bias=True)
        final_dim = final_dim + base_dnn_hidden[-1]

        #self.fc = nn.Linear(final_dim, final_layer)
        self.fc = MLP_Layer(input_dim=final_dim,
                                         output_dim=final_layer, # output hidden layer
                                         hidden_units=[512,64,8],
                                         hidden_activations=dnn_activations,
                                         output_activation=None,
                                         dropout_rates=net_dropout,
                                         batch_norm=batch_norm,
    ...
    def forward(self, inputs):
        ...
        #HEREE
        tmp_dnn = self.base_dnn(flat_feature_emb)
        final_out = torch.cat([final_out, tmp_dnn], dim=-1)
